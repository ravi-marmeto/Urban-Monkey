...
$(document).ready(function() {
      ...
      $("#delivery-form").submit(function(event) {
        ...
        $.getJSON(url, function(data) {
          ...
          // Loop through the data to find the TAT for the given pin code
          $.each(data, function(index, value) {
            if (value.Pincode == pincode) {
              if (paymentType === "cod") {
                tat = parseInt(value.TATCOD);
              } else {
                tat = parseInt(value.TATPRE);
              }
              return false; // Exit the loop once the TAT is found
            }
          });

          if (tat !== null) {
            // Calculate the estimated delivery date based on the current date and TAT
            var currentDate = new Date();
            var deliveryDate = new Date(currentDate.getTime() + (tat * 24 * 60 * 60 * 1000));

            // Display the estimated delivery date to the user
            $("#delivery-date").text("Order now & the product will reach you by " + deliveryDate.toDateString() + ".");
            $("#error-msg").text("");
          } else {
            // Display an error message if the pin code is not found in the data
            $("#delivery-date").text("We could not find any information for the pin code " + pincode + ".");
            $("#error-msg").text("");
          }
        });
      });
